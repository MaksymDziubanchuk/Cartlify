...
export function buildResetPasswordTemplate(args: ResetPasswordTemplateArgs): {
  subject: string;
  text: string;
  html: string;
} {
  const appName = args.appName?.trim() || 'Cartlify';
  const resetUrl = args.resetUrl?.trim() || '';
  const userName = args.userName?.trim() || '';
  const logoCid = args.logoCid?.trim() || '';
  const expiresInText = args.expiresInText?.trim() || '';

  if (!resetUrl) throw new Error('RESET_URL_REQUIRED');
  ...
  const preheader = `Reset your password for ${appName}.`;
...
<html lang="en">
  <head>
    ...
    <title>${safeApp} — Reset password</title>
  </head>
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="border-collapse:collapse;max-width:600px;width:100%;">
            ...
            <tr>
              <td style="background:#ffffff;border:1px solid ${cardBorder};border-radius:14px;padding:28px;font-family:Arial,Helvetica,sans-serif;">
                <div style="font-size:18px;font-weight:800;color:${title};margin:0 0 10px 0;">Reset your password</div>
