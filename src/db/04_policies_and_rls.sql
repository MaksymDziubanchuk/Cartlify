...
REVOKE ALL ON FUNCTION cartlify.set_current_context(cartlify."Role", integer, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION cartlify.set_current_context(cartlify."Role", integer, uuid)
TO cartlify_owner, cartlify_app;

-- Get actor user id
CREATE OR REPLACE FUNCTION cartlify.current_actor_id()
RETURNS integer
LANGUAGE plpgsql
STABLE
AS $$
...
-- Get guest id
CREATE OR REPLACE FUNCTION cartlify.current_guest_id()
RETURNS uuid
LANGUAGE plpgsql
STABLE
AS $$
...
-- Get actor role
CREATE OR REPLACE FUNCTION cartlify.current_actor_role()
RETURNS cartlify."Role"
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
...
-- Is actor admin
CREATE OR REPLACE FUNCTION cartlify.is_admin()
RETURNS boolean
LANGUAGE plpgsql
STABLE
AS $$
...
-- Is actor root
CREATE OR REPLACE FUNCTION cartlify.is_root()
RETURNS boolean
LANGUAGE plpgsql
STABLE
AS $$
...
-- Is actor owner
CREATE OR REPLACE FUNCTION cartlify.is_owner(p_user_id integer)
RETURNS boolean
LANGUAGE plpgsql
STABLE
AS $$
...
-- Is owner or admin
CREATE OR REPLACE FUNCTION cartlify.is_owner_or_admin(p_user_id integer)
RETURNS boolean
LANGUAGE plpgsql
STABLE
AS $$
...
FOR UPDATE
USING (
  cartlify.is_owner(id) OR cartlify.is_root()
)
WITH CHECK (
  (cartlify.is_owner(id) AND role = cartlify.current_actor_role())
  OR cartlify.is_root()
);
...
WITH CHECK (
  role = 'USER'
  OR (cartlify.is_root() AND role IN ('USER', 'ADMIN'))
);
...
CREATE POLICY user_tokens_delete
ON cartlify.user_tokens
FOR DELETE
USING (
  cartlify.is_owner_or_admin("userId")
...
-- UPDATE (deny for all)
CREATE POLICY login_logs_update_deny
ON cartlify.login_logs
FOR UPDATE
USING (false)
WITH CHECK (false);
...
-- DELETE (deny for all)
CREATE POLICY login_logs_delete_deny
ON cartlify.login_logs
FOR DELETE
USING (false);
...
ON cartlify.order_items
FOR SELECT
USING (
  EXISTS (
    SELECT 1
    FROM cartlify.orders o
    WHERE o.id = "orderId"
...
ON cartlify.chat_messages
FOR SELECT
USING (
  EXISTS (
    SELECT 1
    FROM cartlify.chat_threads t
    WHERE t.id = "threadId"
      AND (
...
-- SELECT (admin/root)
CREATE POLICY ppcl_select
ON cartlify.product_price_change_logs
FOR SELECT
USING (
  cartlify.is_admin()
);
...
-- UPDATE (deny for all)
CREATE POLICY ppcl_update_deny
ON cartlify.product_price_change_logs
FOR UPDATE
USING (false)
WITH CHECK (false);
...
-- DELETE (deny for all)
CREATE POLICY ppcl_delete_deny
ON cartlify.product_price_change_logs
FOR DELETE
USING (false);
